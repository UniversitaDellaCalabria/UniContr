...
<p class="normal">
    ...
    {{$pre->settore}}, {{$pre->ore}} ore {{$pre->ore_desc}}, corso di laurea in {{$pre->cdl}} per l'anno accademico {{$pre->aa}};
</p>
@else
<p class="normal">
    - che per il precedente anno accademico è stato {{$pre->rinnovo['storico']}} {{$pre->genere['str4']}} {{$pre->genere['str5']}},
    tramite contratto di diritto privato, lo svolgimento del corso integrativo dell'insegnamento ufficiale di {{$pre->insegnamentoDescr}}
   {{$pre->settore}}, {{$pre->ore}} ore {{$pre->ore_desc}}, corso di laurea in {{$pre->cdl}} presso il {{$pre->dipartimento}};
</p>
<p class="normal">
    ...
    {{$pre->ore}} ore {{$pre->ore_desc}}, corso di laurea in {{$pre->cdl}}, {{$pre->rinnovo['text_rinnovo_2']}} per il precedente anno accademico;
</p>
@endif
...
didattica all'insegnamento ufficiale di {{$pre->insegnamentoDescr}}{{$pre->settore}}, {{$pre->ore}} ore {{$pre->ore_desc}},
...
di {{$pre->insegnamentoDescr}}{{$pre->settore}}, {{$pre->ore}} ore {{$pre->ore_desc}}, corso di laurea in {{$pre->cdl}}
presso il {{$pre->dipartimento}};
</p>
<p class="normal">
...
alla didattica all'insegnamento ufficiale di {{$pre->insegnamentoDescr}}{{$pre->settore}}, {{$pre->ore}} ore {{$pre->ore_desc}},
