...
class Saml2Controller extends Controller
{
    ...
    {
        $baseUri = env('SAML2_IDP_HOST') . env('SAML2_IDP_SIGN_ON_URL');
        $fullUri = $baseUri . '?' . http_build_query($request->query());
        $samlParameters = $request->query();

        $codedRequest = base64_decode($_GET['SAMLRequest']);
        $decodedRequest = gzinflate($codedRequest);
        $samlParameters['SAMLRequest'] = base64_encode($decodedRequest);
    ...
    protected function replaceDestinationUri($samlParameters)
    {
        ...
        $replacedSamlData = str_replace(route('saml_sso_form'), env('SAML2_IDP_HOST') . env('SAML2_IDP_SIGN_ON_URL'),
