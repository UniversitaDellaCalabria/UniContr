...
class User extends Authenticatable implements JWTSubject
{
    ...
    public function getIntendedUrl(){
        if (App::environment('local')) {
            return config('unical.client_url'); //'http://localhost:4200';
        }
        return config('unical.client_url');
    }
    ...
    {
        if($input != '') {
            $this->attributes['blocked_date'] = Carbon::createFromFormat(config('unical.date_format'), $input)->format('Y-m-d');
        }else{
